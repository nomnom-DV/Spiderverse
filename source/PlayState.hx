...
class PlayState extends MusicBeatState
{
	...
	public static var isSpoilerSong:Bool = true;
	...
	override public function create()
	{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				default:
					if(skipCountdownParam) startCountdown();
					if(isSpoilerSong) openSubState(new SpoilerWarning());
					else startCountdown();
			}
			seenCutscene = true;
		}
	...
	public var skipCountdownParam:Bool = false;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			script.executeFunc("goodNoteHit",[notes.members.indexOf(note), leData, leType, isSus]);
			if (!note.isSustainNote)
			{
				if(modchartObjects.exists('note${note.ID}'))modchartObjects.remove('note${note.ID}');
				note.kill();
	...
	public function startScript()
		{
			...
			if (hxdata != "")
			{
				...
				script.setVariable("ShaderFilter",openfl.filters.ShaderFilter);
